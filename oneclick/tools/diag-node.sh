...
if [[ -f /etc/xray/config.json ]]; then
  ...
  echo "VLESS 当前 clients 数量: $(jq -r '.inbounds[]? | select(.tag=="in-vless-reality") | .settings.clients | length' /etc/xray/config.json 2>/dev/null || echo '0')"
...
echo "  - [4] VLESS clients 数量是否 > 0（若为 0 说明 gRPC 未成功写入用户）"
